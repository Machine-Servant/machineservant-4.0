---
date: '2022-11-12'
title: 'Untangling Remix Optimistic UI With Multiple Destinations'
author: 'Evan Stern'
...
published: true
relatedPosts:
  ['2022-11-11-remix-musings', '2023-01-26-remix-react-framework-2023']
---
  ...
  return (
    <div className="flex h-full min-h-screen flex-col">
      ...
      <main className="flex h-full bg-white">
        ...
        <div className="flex-1 p-6">
          // Wrap the Outlet in a loading div
  ...
  return (
    <div className="flex h-full min-h-screen flex-col">
      ...
      <main className="flex h-full bg-white">
        ...
        <div className="flex-1 p-6">
          // While we're loading, display the optimistic UI
      ...
      {data.noteListItems.map((note) => (
        <li key={note.id}>
          <NavLink
            ...
            to={note.id}
            // We add some state so we can figrure out what UI to show
            state={{ page: 'note' }}
          >
            📝 {note.title}
          </NavLink>
...
export const Note: React.FC<NoteProps> = ({ data }) => {
  // We make sure that our component can handle null or undefined data
  return (
    <div>
      ...
      <div className="flex items-center gap-4">
        <Link
          ...
          // And we make sure we add state so we know what optimistic UI to show
...
type TransitionLocationState = {
  page: 'note' | 'edit-note';
  // Add more meta, because meta is fun!
  meta?: {
    [key: string]: string;
  }
};
